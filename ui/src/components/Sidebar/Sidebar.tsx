...
const Sidebar = ({ activeView, collapsed, setCollapsed }: SidebarProps) => {
    ...
    return (
        ...
        >
            <div className="p-3 border-b border-border-color-dark flex justify-end">
                <button
                    onClick={() => setCollapsed(!collapsed)}
                    title={collapsed ? 'Expand' : 'Collapse'}
                    className="p-1.5 rounded-md hover:bg-slate-700 text-gray-300 transition-colors cursor-pointer"
                >
            ...
            <div className="py-4 flex flex-col justify-between h-[calc(100%-60px)]">
                ...
                <div className="px-2 space-y-1">
                    ...
                    {secondaryMenuItems.map((item) => (
                        <button
                            key={item.label}
                            className="w-full flex items-center space-x-3 px-3 py-2.5 rounded-lg text-gray-400 hover:bg-slate-800 transition-all group"
                        >
